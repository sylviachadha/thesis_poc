# PART A 50 normal and 50 abnormal - (Train only normal)
# --------------------------------------------------------------
...
# BALANCED (Model fit on only normal class, predict on different dataset)
...
# Can see sine wave from index 0 to 49 and abnormal wave from
# index 50 to 99
X_2D_test_plot = X_2D_df_test.iloc[45]
plt.plot(X_2D_test_plot)
plt.show()


# Create y_1D (labels) for verification of prediction results of
# 50 normal and 50 abnormal samples
y_class0 = np.zeros(50)
y_class1 = np.ones(50)
y = np.concatenate((y_class0, y_class1))
y_1D = y
...
# Single Plot
anomaly_df_test.iloc[4].plot()
plt.show()

# Concatenate test Data to input to Algorithm X_2D_test
# --------------------------------------------------------
# Create x_2D
# Change df and anomaly_df & concatenate
df_arr_test = df_test.to_numpy()
anomaly_df_arr_test = anomaly_df_test.to_numpy()
...
# PART C 50 normal and 15 abnormal - (Train only normal)
# --------------------------------------------------------------
# Aim - iforest method of anomaly detection on simulated data
# --------------------------------------------------------------#
# Reference https://heartbeat.fritz.ai/isolation-forest-algorithm-for-anomaly-detection-2a4abd347a5
...
# Step 2B - Draw 15 abnormal random pattern
# --------------------------------------------------------------#
...
# Can see sine wave from index 0 to 49 and abnormal wave from
# index 50 to 99
X_2D_test_plot = X_2D_df_test.iloc[56]
plt.plot(X_2D_test_plot)
plt.show()
