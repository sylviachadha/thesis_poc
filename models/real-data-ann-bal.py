...
ann.add(tf.keras.layers.Dense(units=20, activation='relu'))  # Shallow NN i/p n hidden layer
ann.add(tf.keras.layers.Dense(units=20, activation='relu'))  # Now deep NN hidden layer added
...
ann.fit(X_train_short_V5, y_train_short_num, batch_size=30, epochs=80)
...
merged_result = np.concatenate(
    (y_pred_ann1.reshape(len(y_pred_ann1), 1), y_test_short_num_arr.reshape(len(y_test_short_num_arr), 1)), 1)
